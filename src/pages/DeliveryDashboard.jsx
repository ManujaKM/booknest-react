    ...
    <div className="min-h-screen bg-[#0d0d1a] text-white font-['Inter']">
      ...
      <main className="lg:pl-64">
        <header className="flex h-20 items-center justify-between border-b border-white/10 bg-[#0d0d1a]/80 px-8 backdrop-blur-md sticky top-0 z-20">
          <h1 className="text-xl font-bold">Delivery Panel</h1>
                  ...
                  <div key={delivery.id} className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur transition hover:border-purple-500/30 group">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div className="flex gap-4">
                        ...
                        <div>
                          <h3 className="text-lg font-bold">{delivery.customer}</h3>
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <span className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-widest border ${delivery.status === 'pending' ? 'border-amber-500/30 bg-amber-500/10 text-amber-400' :
                    ...
                    <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-white/5 pt-6">
                      ...
                      <div className="flex gap-3">
                        <button className="flex items-center gap-2 rounded-xl bg-white/5 px-4 py-2 text-sm font-medium text-gray-300 hover:bg-white/10">
                          <Phone size={16} />
                          Call
                        </button>
                        <button className="flex items-center gap-2 rounded-xl bg-white/5 px-4 py-2 text-sm font-medium text-gray-300 hover:bg-white/10">
                        ...
                        {delivery.status === 'pending' && (
                          <button
                            ...
                            className="rounded-xl bg-purple-600 px-6 py-2 text-sm font-bold text-white shadow-lg shadow-purple-500/20 hover:bg-purple-500"
                          >
                            Confirm Pickup
                          </button>
                        )}
                        {delivery.status === 'picked_up' && (
                          <button
                            ...
                            className="rounded-xl bg-emerald-600 px-6 py-2 text-sm font-bold text-white shadow-lg shadow-emerald-500/20 hover:bg-emerald-500"
                          >
                            Mark Delivered
                          </button>
                        )}
